/**
 *  Copyright 2019 Bo Wingo
 *  Edited starting with SmartThings' example Z-Wave Door/Window Sensor
 *
...
metadata {
	definition(name: "Driveway Sensor", namespace: "SmartThings-Bo", author: "bowingo", ocfDeviceType: "x.com.st.d.sensor.contact", runLocally: true, minHubCoreVersion: '000.017.0012', executeCommandsLocally: false, genericHandler: "Z-Wave") {
	...
	tiles(scale: 2) {
		multiAttributeTile(name: "contact", type: "generic", width: 6, height: 4, canChangeIcon: true) {
			tileAttribute("device.contact", key: "PRIMARY_CONTROL") {
				attributeState("open", label: "Vehicle Detected", icon: "st.Transportation.transportation8", backgroundColor: "#e86d13")
				attributeState("closed", label: "No Vehicle Detected", icon: "st.Transportation.transportation8", backgroundColor: "#00A0DC")
			}
		}
...
def sensorValueEvent(value) {
	if (value) {
		createEvent(name: "contact", value: "closed", descriptionText: "$device.displayName No Vehicle Detected")
	} else {
		createEvent(name: "contact", value: "open", descriptionText: "$device.displayName Vehicle Detected")
	}
}
